...
public class MorePotions implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        if (MorePotionsConfig.LEVITATION_POTION_ENABLED) {
            registerPotionRecipe(Potions.AWKWARD, Items.SHULKER_SHELL, ModPotions.LEVITATION_POTION);
        ...
        if (MorePotionsConfig.DECAY_POTION_ENABLED) {
            registerPotionRecipe(Potions.AWKWARD, Items.WITHER_ROSE, ModPotions.DECAY_POTION);
        ...
        if (MorePotionsConfig.NAUSEA_POTION_ENABLED) {
            registerPotionRecipe(Potions.AWKWARD, Items.POISONOUS_POTATO, ModPotions.NAUSEA_POTION);
        ...
        if (MorePotionsConfig.LUCK_POTION_ENABLED) {
            registerPotionRecipe(Potions.THICK, Items.RABBIT_FOOT, Potions.LUCK);
            registerLuckModifiers();
        }

        if (MorePotionsConfig.GLOWING_POTION_ENABLED) {
            registerPotionRecipe(Potions.AWKWARD, Items.GLOW_INK_SAC, ModPotions.GLOWING_POTION);
        ...
        if (MorePotionsConfig.BLINDNESS_POTION_ENABLED) {
            registerPotionRecipe(Potions.AWKWARD, Items.INK_SAC, ModPotions.BLINDNESS_POTION);
            registerStandardModifiers(ModPotions.BLINDNESS_POTION, ModPotions.LONG_BLINDNESS_POTION, null, false);
        }
    }

    private static void registerPotionRecipe(Potion input, Item ingredient, Potion output) {
    ...
    private static void registerStandardModifiers(Potion base, Potion extended, Potion amplified, boolean canAmplify) {
        registerPotionRecipe(base, Items.REDSTONE, extended);

        if (canAmplify && amplified != null) {
            registerPotionRecipe(base, Items.GLOWSTONE_DUST, amplified);
        }
    }
