...
public final class ModPotions {
    ...
    private static RegistryEntry<Potion>[][] createLuckVariants() {
        RegistryEntry<Potion>[][] variants = new RegistryEntry[5][4];
    ...
    public static RegistryEntry<Potion> getLuckPotion(int level, int extensionTier) {
        if (level < 1 || level > 5) {
    ...
    private static RegistryEntry<Potion> registerPotion(String id, Potion potion) {
        return Registry.registerReference(Registries.POTION, Identifier.of(MorePotions.MOD_ID, id), potion);
    }
